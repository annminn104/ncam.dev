...
/**
 * Hyper Rare, the gold tier, ported from pokemon-cards-151's hyper-rare.css
 * (css.ts has the conversions and the approximations every port shares). The
 * gold is the card's own printing: the reference adds no gold of its own, and
 * its CSS is ex-special-illustration-rare.css's with one number changed, the
...
export const hyperRare: Effect = {
  ...
  shine: [
    // .card__glitter, beneath the shine
    {
      ...
      opacity: { base: 0.2, fromCenter: 0.3 },
    },
